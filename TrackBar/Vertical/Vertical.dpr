...
const
  ...
  AppName = 'Trackbar-Demo (Vertical)';
  WindowWidth = 240;
  WindowHeight = 360;
  ...
  case uMsg of
    ...
    begin
      ...
      hredTB := CreateWindowEx(0, 'msctls_trackbar32', '', WS_VISIBLE or WS_CHILD
        or WS_TABSTOP or TBS_TOP or TBS_AUTOTICKS or TBS_TOOLTIPS or TBS_VERT, 40, 15, 40, 250, hWnd, 0, hInstance, nil);
      ...
      hgreenTB := CreateWindowEx(0, 'msctls_trackbar32', '', WS_VISIBLE or WS_CHILD
        or WS_TABSTOP or TBS_BOTH or TBS_AUTOTICKS or TBS_TOOLTIPS or TBS_VERT, 100, 15, 40, 250, hWnd, 0, hInstance, nil);
      ...
      hblueTB := CreateWindowEx(0, 'msctls_trackbar32', '', WS_VISIBLE or WS_CHILD
        or WS_TABSTOP or TBS_AUTOTICKS or TBS_TOOLTIPS or TBS_VERT, 160, 15, 40, 250, hWnd, 0, hInstance, nil);
    ...
    begin
      ...
      lstrcat(s , color);
      TextOut(dc, 15, 300, s, lstrlen(s));
      lstrcpy(s, 'Grün: ');
      wvsprintf(color, '%d', PChar(@green));
      lstrcat(s , color);
      TextOut(dc, 80, 300, s, lstrlen(s));
      lstrcpy(s, 'Blau: ');
      wvsprintf(color, '%d', PChar(@blue));
      lstrcat(s , color);
      TextOut(dc, 155, 300, s, lstrlen(s));

      EndPaint(hWnd, ps);
    end;
    WM_VSCROLL:  // wird bei Veränderung der Slider-Position gesendet
    begin
